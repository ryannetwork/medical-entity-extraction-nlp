...
label_index_dict = {}     # Label Dictionary - Labels to Index
reverse_label_index_dict = {}   # Inverse Label Dictionary - Index to Labels

def initialize_labels(s_path):     # Initializing label dictionaries for Labels->IDX and IDX->Labels
	# Using BIEOS labelling scheme
	label_index_dict['problem_b'] = 0     # Problem - Beginning 
	label_index_dict['problem_i'] = 1     # Problem - Inside
	label_index_dict['problem_e'] = 2     # Problem - End
	label_index_dict['problem_s'] = 3     # Problem - Single
	label_index_dict['test_b'] = 4        # Test - Beginning
	label_index_dict['test_i'] = 5        # Test - Inside
	label_index_dict['test_e'] = 6        # Test - End
	label_index_dict['test_s'] = 7        # Test - Single
	label_index_dict['treatment_b'] = 8   # Treatment - Beginning
	label_index_dict['treatment_i'] = 9   # Treatment - Inside
	label_index_dict['treatment_e'] = 10  # Treatment - End
	label_index_dict['treatment_s'] = 11  # Treatment - Single
	label_index_dict['o'] = 12            # Outside Token

	# Making Inverse Label Dictionary
	for k in label_index_dict.keys():
	    reverse_label_index_dict[label_index_dict[k]] = k

	# Saving the dictionaries into a file
	save_data([label_index_dict, reverse_label_index_dict], os.path.join(s_path, "label_dicts.dat"))
...
def parse_concepts(file_path):      # Parses the concept file to extract concepts and labels
	...
	for x in content:               # Reading each line in the concept file
		...
		label_indexes = [label_index_dict[l] for l in lab]
		start_line, start_col = int(temp1[-2].split(":")[0]), int(temp1[-2].split(":")[1])
		end_line, end_col = int(temp1[-1].split(":")[0]), int(temp1[-1].split(":")[1])
		...
		print("Labels  Index: " + str(label_indexes))
		print("Start Line: " + str(start_line) start_c", Start Column: " + str(sCol))
		print("End Line: " + str(end_line) + ", End Column: " + str(end_col))
		print("------------------------------------------------------------")
		'''
		...
		dic['label_index'] = label_indexes  # Labels in the index form
		dic['start_line'] = start_line  #Start line of the concept in the corresponding text summaries
		dic['start_word_no'] = start_col # Starting word number of the concept in the corresponding start line
		dic['end_line'] = end_line     # End line of the concept in the corresponding text summaries
		dic['end_word_no'] = end_col   # Ending word number of the concept in the corresponding end line
...
def print_data(file, file_lines, tags):       # Prints the given data
	...
	for x in file_lines:
		...
		print(x)
		print("\nCorresponding labels:")
		print([reverse_label_index_dict[i] for i in tags[counter-1]])
...
if __name__ == '__main__':
	...
	# File paths
	save_path = constants.CLEANED_FILES_PATH
	concept_path = constants.PARTNER_CONCEPT_FILES_PATH
